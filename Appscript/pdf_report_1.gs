...
//============================================//
//========= Import Data (importrange) ========//
//============================================//

function importRange() {  

  // clear content in exsisting sheet
  const sheet = SpreadsheetApp.getActive().getSheetByName('query-new')
  sheet.getRange('A:AK').clearContent()

  // set importrange from another file with parameter
  const dstCell = sheet.getRange("A1");
  const fileId ="1rCqRQ8fQ0Zb-N414Ixddd5LkW9v0Tdw1JL2-NTZ5jpQ";
  const targetSheet_1= "EmonVLOOKUP!$A:$AK";
  const targetSheet_2= "EmonVLOOKUP!$a:$AK";
  const targetSheet_3= "EmonVLOOKUP!$A:$ak";
  const targetSheet_4= "EmonVLOOKUP!$a:$ak";
  const assignFormula = 'IFERROR(IMPORTRANGE("'+fileId+'";"'+targetSheet_1+'");IFERROR(IMPORTRANGE("'+fileId+'";"'+targetSheet_2+'");IFERROR(IMPORTRANGE("'+fileId+'";"'+targetSheet_3+'");IMPORTRANGE("'+fileId+'";"'+targetSheet_4+'"))))'

  dstCell.setFormula(assignFormula);

}

function freezeData() {

// copy values only for importrange result
  const sheet = SpreadsheetApp.getActive().getSheetByName('query-new')
  const range = sheet.getRange('A:AK')
